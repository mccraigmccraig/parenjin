...
(with-state-changes [(around :facts (let [result (promise)
  ...
  (fact "start-or-noop should start a never started task"
    (start-or-noop ..enjin.. trackref (fn [enjin] enjin => ..enjin.. (deliver result ..result..) (Thread/sleep 50))) => :running
...
(with-state-changes [(around :facts (let [result (promise)
  ...
  (fact "start-or-noop should do nothing to a running task"
    (start-or-noop ..enjin.. trackref (fn [enjin] (deliver result ..fail..))) => :running
...
(with-state-changes [(around :facts (let [b (promise)
  ...
  (fact "start-or-noop should start a stopped task"
    @b => ..before-result..
    (start-or-noop ..enjin.. trackref (fn [enjin] enjin => ..enjin.. (deliver a ..after-result..) (Thread/sleep 50))) => :running
